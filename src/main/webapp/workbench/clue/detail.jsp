    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#searchA').on("input", function (e) {
                showActivityList($.trim($('#searchA').val()))
            })

            $('#addRelation').click(function () {
                var $ckb = $('input[name="activityCkb"]:checked')

                var param = "clueID=" + "${clue.id}&"
                for (var i = 0; i < $ckb.length; i++) {
                    param += "activityID=" + $ckb.eq(i).val();
                    if (i < $ckb.length - 1) {
                        param += "&";
                    }
                }
                ...
                $.ajax({
                    url: "workbench/clue/addDetailRelation.do",
                    data: param,
                    type: "post",
                    dataType: "json",
        ...
        function showRelationList() {
            ...
            $.ajax({
                url: "workbench/clue/getDetailRelation.do",
                data: {
                    "clueID": "${clue.id}"
                },
                type: "get",
                dataType: "json",
                success: function (data) {
                    var activityListBody = $('#activityListBody');
                    $.each(data, function (i, v) {
                            ...
                            html += '<td>' + v.owner + '</td>';
                            html += '<td><a href="javascript:void(0);" onclick="removeRelation(\'' + v.id + '\')" style="text-decoration: none;">';
        ...
        function showActivityList(name) {
            if (name === undefined) {
                name = "";
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    var html = "";
                    if (data.success) {
                        $.each(data.activityList, function (i, v) {

                            html += '<tr>';
                            html += '<td><input type="checkbox" name="activityCkb" value="' + v.id + '"/></td>';
                            html += '<td>' + v.name + '</td>';
                            html += '<td>' + v.startDate + '</td>';
                            html += '<td>' + v.endDate + '</td>';
                            html += '<td>' + v.owner + '</td>';
                ...
                <div class="btn-group" style="position: relative; top: 18%; left: 8px;">
                    <form class="form-inline" role="form">
                        <div class="form-group has-feedback">
                            <input type="text" class="form-control" id="searchA" style="width: 300px;"
...
<div style="position: relative; left: 40px; top: -30px;">
    <div class="page-header">
        <h3>${clue.fullname}${clue.appellation} <small>ALsW</small></h3>
    </div>
...
<div>
    <div style="position: relative; top: 60px; left: 40px;">
        ...
        <div>
            <a href="javascript:void(0);" onclick="showActivityList()"
